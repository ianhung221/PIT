...
export function VehicleMirrors({ runtime, mode, quality = "high" }: {
  ...
  useFrame(() => {
    ...
    const offsets = [0, -.36, .36];
    mirrorCameras.forEach((mirrorCamera, index) => {
      const angle = car.yaw + offsets[index];
      mirrorCamera.position.set(car.x, .9, car.z);
      look.set(car.x + Math.sin(angle) * 28, .65, car.z + Math.cos(angle) * 28);
  ...
  return <group ref={group} visible={false}>
    <mesh position={[0, .25, 0]}><planeGeometry args={[.62, .16]} /><meshBasicMaterial map={targets[0].texture} toneMapped={false} /></mesh>
    {qualitySettings.sideMirrors && <>
      <mesh position={[-.48, -.02, 0]} rotation={[0, .16, 0]}><planeGeometry args={[.25, .15]} /><meshBasicMaterial map={targets[1].texture} toneMapped={false} /></mesh>
      <mesh position={[.48, -.02, 0]} rotation={[0, -.16, 0]}><planeGeometry args={[.25, .15]} /><meshBasicMaterial map={targets[2].texture} toneMapped={false} /></mesh>
    </>}
  </group>;
}
